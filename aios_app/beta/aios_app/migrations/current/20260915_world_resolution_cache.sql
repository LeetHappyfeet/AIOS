-- Restore the materialized world/domain resolution subsystem for canonical
-- baseline installs. The subsystem originally lived in a historical migration
-- that is not replayed by the fresh-install migrator, while later current
-- migrations depend on these objects.
