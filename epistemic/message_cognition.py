...
def _canonical_text(candidate: ParsedCandidate, *, owner: str | None) -> str:
    ...
    predicate = _NEGATED_VERBS.get(
        candidate.predicate.lower().strip(), candidate.predicate.lower().strip()
    )
    if candidate.kind == "MEMORY":
        return f"{subject} remembers {obj}."
    if candidate.kind == "BELIEF":
        if predicate in {"is", "was", "seems", "seemed"} and obj.startswith(
            ("uncertain", "unsure", "confused")
        ):
...
def _parse_sentence(sentence: str) -> ParsedCandidate | None:
    ...
    if match:
        return ParsedCandidate(
            "MEMORY", match.group("subject"), match.group("verb"), match.group("object"), 0.94, "memory_predicate"
        )
    match = _BELIEF_RE.search(sentence)
    if match:
        return ParsedCandidate(
            "BELIEF", match.group("subject"), match.group("verb"), match.group("object"), 0.92, "belief_predicate"
        )
    match = _UNCERTAIN_RE.search(sentence)
    if match:
        obj = f"{match.group('object')}{match.group('tail') or ''}"
        return ParsedCandidate(
            "BELIEF", match.group("subject"), match.group("verb"), obj, 0.90, "uncertainty_state"
        )
    match = _RULE_RE.search(sentence)
    if match:
        return ParsedCandidate(
            "RULE", match.group("subject"), match.group("verb"), match.group("object"), 0.93, "deontic_predicate"
        )
    ...
    if match and _STATE_TERMS_RE.search(match.group("object")):
        return ParsedCandidate(
            "STATE", match.group("subject"), match.group("verb"), match.group("object"), 0.86, "bounded_state_predicate"
        )
...
def _score_candidate(
    candidate: ParsedCandidate, *, character_owned: bool, index: int, total: int
) -> float:
    ...
    *,
    ...
    for index, sentence in enumerate(sentences):
        ...
        salience = _score_candidate(
            candidate, character_owned=character_owned, index=index, total=len(sentences)
        )
