...
def _viewpoint_id(message: dict[str, Any], speaker_type: str) -> str | None:
    speaker_id = str(message.get("name") or "").strip() or None
...
def parse_sillytavern_jsonl(path: Path) -> ParsedChatLog:
    ...
    for index, raw in enumerate(rows[1:]):
        ...
        speaker_id = str(raw.get("name") or "").strip()
        ...
        messages.append(
            ParsedMessage(
                ...
                viewpoint_id=_viewpoint_id(raw, speaker_type) or (
                    speaker_id if speaker_type in {"user", "character"} else None
                ),
                text=text,
                event_time=_parse_time(raw.get("send_date")),
                raw=raw,
            )
        )

    if not messages:
        raise ValueError("No SillyTavern message records containing 'mes' were found")
...
def summarize_chat_log(path: Path) -> dict[str, Any]:
    ...
    user_messages = sum(1 for item in parsed.messages if item.speaker_type == "user")
    character_messages = sum(
        1 for item in parsed.messages if item.speaker_type == "character"
    )
    system_messages = sum(1 for item in parsed.messages if item.speaker_type == "system")
    return {
        "file": path.name,
