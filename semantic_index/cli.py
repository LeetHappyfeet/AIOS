...
from .admission import admit_semantic_neighbors_once
...
def _semantic_snapshot(*, state: str, stage: str, stage_started_at: float | None,
                       ...
                       totals: dict[str, int], window_started: float) -> dict[str, Any]:
...
async def run_forever(poll_seconds: float = 1.0) -> None:
    ...
    try:
        ...
        totals = {key: 0 for key in (
            ...
            "admitted", "structured", "neighbor_classified", "clustered", "classified", "reconciled"
        )}
        last_batches = {"source": 0, "frames": 0, "propositions": 0, "epistemic": 0, "admission": 0}
        ...
        while True:
            ...
            neighbor_classified = await run_stage("neighbor-classification", classify_neighbor_relations_once, db, cfg)
            ...
            classified = await run_stage("cluster-classification", classify_latest_clusters_once, db, cfg)
            totals["classified"] += int(classified)
            reconciled = await run_stage("reconciliation", reconcile_semantic_structure_once, db, fuseki, cfg)
            ...
            window_started = time.monotonic()
            if indexed == 0 and admitted == 0 and structured == 0 and neighbor_classified == 0 and clustered == 0 and classified == 0 and reconciled == 0:
