...
async def classify_semantic_admission(
    ...
    *,
    ...
    current = await db.fetchrow(
        ...
        FROM aios.proposition WHERE proposition_id=$1
        """,
        proposition_id,
    )
    if not current:
...
async def _record_decision(
    ...
    *,
    ...
    await db.execute(
        ...
        decision.reason,
        '{"candidate_source":"qdrant_canonical_proposition"}',
    )

    if decision.decision != "reinforces":
        return

    # Preserve the SQL observation/evidence but prevent duplicate permanent RDF
    # observation and claim-topology expansion. These receipts use the exact
    # downstream identities already consumed by the supervisor.
    await db.execute(
        """
        INSERT INTO aios.rdf_promotion_log (
            ...
            $1,'world','urn:aios:world:epistemic',
            'urn:aios:world:observation:' || $1::text,
            'world:observesProposition','suppressed:semantic-reinforcement',
            'qdrant_semantic_admission_gate',now(),
        ...
        DO UPDATE SET rdf_object=EXCLUDED.rdf_object,
    ...
    await db.execute(
        ...
        INSERT INTO aios.semantic_topology_projection (
            ...
            'claim:' || $1::text || ':semantic-reinforcement',$1,$2,'none',
            'urn:aios:suppressed:semantic-reinforcement','semantic-topology-v1',
            now(),NULL,
...
async def admit_semantic_neighbors_once(
    db: Database,
    cfg: SemanticIndexConfig,
    *,
    embedder: Embedder,
    ...
    rows = await db.fetch(
        ...
        JOIN aios.semantic_vector_index_state svi
          ON svi.object_type='proposition'
         AND svi.object_key=o.proposition_id::text
         AND svi.qdrant_collection=$2
         AND svi.embedding_model=$3
         AND svi.embedding_version=$4
